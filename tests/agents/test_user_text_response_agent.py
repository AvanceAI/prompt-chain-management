...
def user_text_response_agent():
    input_resolver = AsyncMock()
    agent_params = {
        "dependencies": ["dep1", "dep2"],
        "message": "Please enter input:"
    }
    return UserTextResponseAgent(agent_params=agent_params, input_resolver=input_resolver)
...
async def test_execute(user_text_response_agent):
    ...
    user_text_response_agent.input_resolver.get_user_response.return_value = user_input

    # Act
    response = await user_text_response_agent.execute(None)  # variable_store is not used in execute

    # Assert
    user_text_response_agent.input_resolver.request_user_input.assert_called_once_with(correlation_id, user_text_response_agent.agent_params.message)
    user_text_response_agent.input_resolver.get_user_response.assert_called_once_with(correlation_id)
